//credit to: https://gpfault.net/posts/mapping-types-to-values.txt.html
#ifndef TYPEMAP_H
#define TYPEMAP_H
...
/*   template <class Key>
  iterator at() { return m_map.at(getTypeId<Key>()); }

  template <class Key>
  const_iterator at() const { return m_map.at(getTypeId<Key>()); }

  template <class Key>
  ValueType &operator[](const Key&){ return m_map[getTypeId<Key>()]; } */

  // Associates a value with the type "Key"
  template <class Key>
  std::pair<iterator, bool> put(ValueType &&value) {
    return m_map.insert(std::make_pair<int, ValueType>(getTypeId<Key>(), std::forward<ValueType>(value)));
  }  
